...
// LimitPolicy configures rate limiting.
...
// EstimatorConfig configures token estimation.
type EstimatorConfig struct {
	Type  string  `yaml:"type" json:"type"`   // e.g. "length_ratio", "tiktoken"
	Ratio float64 `yaml:"ratio" json:"ratio"` // chars-to-tokens ratio
}

// SlidingWindow is a single window limit.
type SlidingWindow struct {
	Threshold      int64    `yaml:"threshold" json:"threshold"`
	TimeWindowInMs int64    `yaml:"time_window_in_ms" json:"time_window_in_ms"`
	BurstRatio     *float64 `yaml:"burst_ratio,omitempty" json:"burst_ratio,omitempty"` // token-bucket burst factor
}
